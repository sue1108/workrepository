...
public class CommonsThread extends Thread {
	...
	protected final Logger logger = LoggerFactory.getLogger(CommonsThread.class);
	
	/**
	 * 线程的运行状态，true:线程run；false：线程wait。默认状态为false
	 */
	...
	public boolean isStateFlag() {
		Object obj[] = {threadName , index , stateFlag};
		logger.info("{} 线程 {} 的状态：{}",obj);
		return stateFlag;
	}
	...
	public void setThreadInitState() throws InterruptedException {
		Object obj[] = {threadName , index , "wait"};
		logger.info("设置{}线程{}的初始状态为{}",obj);
		if (!stateFlag)
			synchronized (this) {
				this.wait();
			}
